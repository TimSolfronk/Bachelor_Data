...
def display_tracer_graphs(tracerPos):
    ...
    dataAmount = max(min(len(csvData[0])-dataIndex,args.v),0)
    ...
    for i in range(0, dataAmount):
        dt, y = get_specific_tracer_data(csvDataForTracer, i)
        currentPlot = axs[i%ROW_AMOUNT, i//ROW_AMOUNT] if columnAmount > 1 else axs[i]
        currentPlot.plot(dt, y, color = 'b', linestyle = 'dashed',
            marker = 'o',label = "Variable " + str(i+1) + " as a function of time")
        currentPlot.set_title("Variable " + str(i+1))
        currentPlot.set_xlabel('t')
        currentPlot.set_ylabel("Variable " + str(i+1))
        currentPlot.legend()
...
def get_tracer_name(tracerPos):
    tracerName = "("
    for coord in str(tracerPos)[1:-1].split(","):
        tracerName += str(round(float((coord.strip())[1:-1]),3)) + ", "
...
if __name__ == "__main__":
    ...
    parser.add_argument("--v",               dest="v",              type=int,   default=99,     help="Amount of variables to be plotted (first v will be plotted)" )
