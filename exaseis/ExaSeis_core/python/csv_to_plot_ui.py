...
SCENARIO_CONFIGS = {
    ...
    "tpv34": [[11,2,9,0,10,1], NAME_CONFIGS["tpv"],True],
...
def display_tracer_graphs(tracerPos):
    ...
    for i in range(dataAmount):
        ...
        currentPlot.plot(dt, y, color = 'b', label = "ExaHyPE 2")#varName + " as a function of time")
        if args.compare:
            for ref_id in range(len(csvRefs)):
                ref_dt, ref_y = get_reference_data(tracerPos,SCENARIO_CONFIGS[args.scenario][-1][i],ref_id)
                if len(ref_dt) == 0:
                    continue
