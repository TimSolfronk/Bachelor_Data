...
def display_tracer_graphs(tracerPos):
    ...
    for i in range(dataAmount):
        ...
        varName = "Variable " + str(varsToPlot[i]+1)
...
def get_tracer_name(tracerPos):
    ...
    for i in range(len(coord_list)):
        if args.has_offset and i == 2:
            tracerName +=str(round(float((coord_list[i].strip())[1:-1])-20.,3)) + ", "
        else:
            tracerName += str(round(float((coord_list[i].strip())[1:-1]),3)) + ", "
